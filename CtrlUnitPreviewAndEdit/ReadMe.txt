================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : CtrlUnitPreviewAndEdit プロジェクトの概要
===============================================================================

この CtrlUnitPreviewAndEdit アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、CtrlUnitPreviewAndEdit アプリケーションを構成する各ファイルの内容の概要が含まれています。

CtrlUnitPreviewAndEdit.vcxproj
...
CtrlUnitPreviewAndEdit.vcxproj.filters
...
CtrlUnitPreviewAndEdit.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CCtrlUnitPreviewAndEditApp アプリケーション クラスを宣言します。

CtrlUnitPreviewAndEdit.cpp
    これはメインのアプリケーション ソース ファイルで、CCtrlUnitPreviewAndEditApp アプリケーション クラスが含まれています。

CtrlUnitPreviewAndEdit.rc
...
res\CtrlUnitPreviewAndEdit.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル CtrlUnitPreviewAndEdit.rc にインクルードされます。

res\CtrlUnitPreviewAndEdit.rc2
...
CtrlUnitPreviewDoc.h, CtrlUnitPreviewDoc.cpp - ドキュメント
    これらのファイルには、CCtrlUnitPreviewDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CCtrlUnitPreviewDoc::Serialize を使用)。

CtrlUnitPreviewView.h, CtrlUnitPreviewView.cpp - ドキュメントのビュー
    これらのファイルには、CCtrlUnitPreviewView クラスが含まれます。
    CCtrlUnitPreviewView オブジェクトは CCtrlUnitPreviewDoc オブジェクトを表示するために使用されます。

res\CtrlUnitPreviewAndEditDoc.ico
    これは、CCtrlUnitPreviewDoc クラスの MDI 子ウィンドウのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル CtrlUnitPreviewAndEdit.rc にインクルードされます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、CtrlUnitPreviewAndEdit.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

CtrlUnitPreviewAndEdit.manifest
