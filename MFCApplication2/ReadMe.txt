================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : MFCApplication2 プロジェクトの概要
===============================================================================

この MFCApplication2 アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、MFCApplication2 アプリケーションを構成する各ファイルの内容の概要が含まれています。
...
MFCApplication2.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CMFCApplication2App アプリケーション クラスを宣言します。

MFCApplication2.cpp
    これはメインのアプリケーション ソース ファイルで、CMFCApplication2App アプリケーション クラスが含まれています。
...
res\MFCApplication2.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル MFCApplication2.rc にインクルードされます。
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、MFCApplication2.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////

アプリケーション ウィザードは 1 つのドキュメントの種類と 1 つのビューを作成します。

MFCApplication2Doc.h, MFCApplication2Doc.cpp - ドキュメント
    これらのファイルには、CMFCApplication2Doc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CMFCApplication2Doc::Serialize を使用)。

MFCApplication2View.h, MFCApplication2View.cpp - ドキュメントのビュー
    これらのファイルには、CMFCApplication2View クラスが含まれます。
    CMFCApplication2View オブジェクトは CMFCApplication2Doc オブジェクトを表示するために使用されます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、MFCApplication2.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。
